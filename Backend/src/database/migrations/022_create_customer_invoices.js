 ...
 *
 * Customer Invoice (project.md §5.2, §7.3). The mirror of
 * 020_create_vendor_bills — a receivable rather than a payable.
 *
 * Status Lifecycle (project.md §5.2.6):
 *   'draft' -> 'posted' -> 'partially_paid' -> 'paid' -> 'overdue' -> 'cancelled'
 *
 * OVERDUE IS DERIVED, NEVER STORED AS A TRANSITION — technicalrequirement.md
 * §7.8. It is a predicate over the row:
 *   status IN ('posted','partially_paid') AND due_date < CURRENT_DATE AND amount_due > 0
 * A nightly job setting an 'overdue' column would duplicate that predicate and
 * drift between runs. The status column still ALLOWS 'overdue' so the two
 * document tables share one CHECK vocabulary, but nothing writes it.
 *
 * The partial index below is what makes the derived filter cheap: the overdue
 * list only ever scans rows that are still open.
 */
...
const UP = `
  CREATE TABLE IF NOT EXISTS customer_invoices (
    ...
    -- A payment can never take amount_paid past the total.
    CONSTRAINT ck_customer_invoices_paid_within_total CHECK (amount_paid <= total_amount)
  );
  ...
  -- Serves the derived-overdue predicate and the portal's "what do I owe"
  -- query, both of which only care about still-open invoices.
  CREATE INDEX IF NOT EXISTS idx_customer_invoices_open_due
    ON customer_invoices(organization_id, due_date)
  ...
  CREATE INDEX IF NOT EXISTS idx_customer_invoice_lines_inv_id
    ON customer_invoice_lines(customer_invoice_id);

  CREATE INDEX IF NOT EXISTS idx_customer_invoice_lines_org_product
    ON customer_invoice_lines(organization_id, product_id);

  CREATE INDEX IF NOT EXISTS idx_customer_invoice_lines_org_analytic
    ON customer_invoice_lines(organization_id, analytic_account_id)
    WHERE analytic_account_id IS NOT NULL;

  CREATE INDEX IF NOT EXISTS idx_customer_invoice_lines_org_income
