  ...
  return (
    <>
      <header className="form-head-auth">
        ...
        <p className="form-sub-auth">{t('login.subtitle')}</p>
      </header>
      ...
      <form className="login-form-auth" onSubmit={handleSubmit} noValidate>
        ...
        {/* Secondary CTA — signing up creates an organization, which is a
            destination in its own right, so it gets a button rather than a
            word-sized link buried in the subtitle. */}
        <div className="alt-action-auth">
          <span className="alt-action-label-auth">{t('login.switchPrompt')}</span>
          {onSwitchToRegister ? (
            <button
              type="button"
              ...
              disabled={isSubmitting}
            >
              <span>{t('login.switchAction')}</span>
            </button>
          ) : (
            <Link href="/auth/register" className="btn-secondary-auth">
              <span>{t('login.switchAction')}</span>
            </Link>
          )}
        </div>
